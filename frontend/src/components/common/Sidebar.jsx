...
const NavItem = ({ to, icon, label, active }) => (
  <Link to={to} className={`flex items-center gap-3 px-4 py-3 mx-2 rounded-lg text-sm font-medium transition-all ${
    active ? 'bg-primary-600 text-white shadow-sm' : 'text-primary-100 hover:bg-primary-700 hover:text-white'
  }`}>
    ...
    <aside className="w-64 bg-primary-800 flex flex-col h-screen fixed left-0 top-0 z-40">
      ...
      <div className="border-t border-primary-700 p-4">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 bg-primary-600 rounded-full flex items-center justify-center text-white font-bold text-sm border-2 border-primary-400">
        ...
        <button onClick={handleLogout} className="w-full text-primary-200 hover:text-white hover:bg-primary-700 py-2 px-3 rounded-lg text-sm transition-colors flex items-center gap-2">
          <span>🚪</span> Sign Out
        </button>
      </div>
    </aside>
